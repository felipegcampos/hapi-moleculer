...
internals.schema = Joi.object({
  name: Joi.string().trim().default('api'),
  broker: Joi.alternatives().try(Joi.object().type(ServiceBroker), Joi.object()).default({}),
  aliases: Joi.array().items(
    Joi.object().keys({
      ...
      blacklist: Joi.alternatives().when('method', {
        is: 'REST',
        then: Joi.array().items(Joi.string().allow(internals.restActions)).unique(),
...
internals.onPreAuth = async function (request, h) {
  ...
  request.ctx = Context.create(request.broker, action, request.broker.nodeID, request.params, {})
